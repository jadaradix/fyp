...
var gimg = require("google-images");
    ...
    async.waterfall(
          ...
          request(
            ...
            function (err, response, body) {
              if (err) {
                ...
                //only 3 mid's
                if (ids.length > 3) {
                  ids = ids.splice(0, ids.length - 3 + 1);
                }
                next(null, ids);
              }
            }
      ...
      function(err, descriptions) {
        var r = {
          "name": humanName,
          "text": null,
          "image": "http://motf.me/static/blank.jpg"
        };

        gimg.search(humanName, function(err2, images) {
          if (!err2 && !(images.length == 0)) {
            r.image = images[0].unescapedUrl;
          }
          if (err) {
